...
function [C, B, t, sigma2, iter, T, history]=cpd_affine(X,Y, max_it, tol, viz, outliers, fgt, corresp, sigma2, savegif, verbosity, saveoptim)
...
while (iter<max_it) && (ntol > tol) && (sigma2 > 10*eps)
    ...
    ntol=abs((L-L_old)/L);
    if verbosity > 0; disp([' CPD Affine ' st ' : dL= ' num2str(ntol) ', iter= ' num2str(iter) ' sigma2= ' num2str(sigma2)]); end
