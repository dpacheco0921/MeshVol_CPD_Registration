...
function  [C, W, sigma2, iter, T, history] =cpd_GRBF(X, Y, beta, lambda, max_it, tol, viz, outliers, fgt, corresp,sigma2, savegif, verbosity, saveoptim)
...
%while (iter<max_it)  && (sigma2 > 1e-8) %(sigma2 > 1e-8)
    ...
    ntol=abs((L-L_old)/L);
    if verbosity > 0; disp([' CPD nonrigid ' st ' : dL= ' num2str(ntol) ', iter= ' num2str(iter) ' sigma2= ' num2str(sigma2)]); end
